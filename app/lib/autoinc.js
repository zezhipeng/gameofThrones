module.exports = new function () {
  ...
  this.plugin = function (schema, options) {
    if (!options.model) throw new Error('Missing required parameter: model')
    ...
    options.once = (!isNaN(options.once)) ? options.once : 0
    ...
    Counter.findOne({
      ...
      if (err) throw new Error('Can not find counter')
      if (!res) {
        (new Counter({
          model: options.model,
          field: options.field,
  ...
  this.init = function (database, counter) {
    ...
    // if (db.counters && db.counters.count() > 0) return
